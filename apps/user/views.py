...
class LoginView(APIView):
    def post(self, request):
        ...
        if user is None:
            LoginAttempt.objects.create(user=None, is_success=False, ip_address=request.META.get("REMOTE_ADDR"))
            return Response({"message": "이메일이나 비밀번호를 확인해주세요"}, status=status.HTTP_401_UNAUTHORIZED)

        if user.is_locked:
            return Response({"message": "정지된 계정입니다."}, status=status.HTTP_403_FORBIDDEN)
    ...
    def post(self, request):
        ...
        if provider == "kakao":
            ...
            if response.status_code != 200:
                return Response({"message": "카카오 토큰 인증 실패"}, status=status.HTTP_401_UNAUTHORIZED)
            data = response.json()
            ...
            response = requests.get(url, headers=headers)
            if response.status_code != 200:
                return Response({"message": "구글 토큰 인증 실패"}, status=status.HTTP_401_UNAUTHORIZED)
            data = response.json()
            ...
            response = requests.get(url, headers=headers)
            if response.status_code != 200:
                return Response({"message": "네이버 토큰 인증 실패"}, status=status.HTTP_401_UNAUTHORIZED)
            data = response.json().get("response", {})
            email = data.get("email")
            name = data.get("nickname", email.split("@")[0] if email else "naver_user")

        else:
            return Response({"message": "지원하지 않는 provider"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response(
            {
                "message": f"{provider} 소셜 로그인이 완료되었습니다.",
                "data": {"access_token": str(refresh.access_token), "refresh_token": str(refresh)},
            },
            status=status.HTTP_200_OK,
        )
...
class LogoutView(APIView):
    ...
    def post(self, request, user_id):
        ...
        return Response({"message": "토큰 인증 실패"}, status=status.HTTP_401_UNAUTHORIZED)
...
class UserEditView(APIView):
    ...
    def put(self, request, user_id):
        ...
        if name and User.objects.exclude(id=user_id).filter(name=name).exists():
            return Response({"message": "닉네임이 중복되었습니다."}, status=status.HTTP_409_CONFLICT)
